...
function DL.num.FormatSecondsAsTime(val, is_hour, is_ms)
    is_hour = (is_hour == nil) and true or is_hour
    is_ms = (is_ms == nil) and true or is_ms
    ...
    if is_ms then
        local ms = (val - seconds) * 1000 // 1
        vals[#vals+1] = ms
    end 
    -- format 
    local format = (is_hour and '%02d:' or '') .. '%02d:'.. '%02d' .. (is_ms and '.%03d' or '')
