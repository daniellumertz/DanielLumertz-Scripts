...
---Create a copy of Item at position and track 
---@param item item
---@param position number
---@param track track
---@return item new_item
function CopyItem(item,position,track) -- Thanks Amagalma https://forum.cockos.com/showthread.php?t=104319 
...
function SelectAutomationItemsInRange(proj,start_range,fim_range,only_start_in_range,only_end_in_range, is_select)
    ...
    for track in enumTracks(proj) do
        for env in enumTrackEnvelopes(track) do
            ...
            for i = 0, cnt-1 do
                ...
                if final_pos > start_range then
                    reaper.GetSetAutomationItemInfo(env, i, 'D_UISEL', is_select, true)
                end
...
---@param only_end_in_range boolean only get items that end inside the range (if end at the fim_range it is includded)
---@param is_select boolean if true will select if false will unselect
function DeleteAutomationItemsInRange(proj,start_range,fim_range,only_start_in_range,only_end_in_range)
    for track in enumTracks(proj) do
        for env in enumTrackEnvelopes(track) do
            local delete_list = {}
            local cnt = reaper.CountAutomationItems(env)
...
function CopyAutomationItemsInfo_Value(env, ai_origem_idx,ai_destino_idx,table_strings)
    for index, info_string in ipairs(table_strings) do
        local ai_origem_value =  reaper.GetSetAutomationItemInfo(env, ai_origem_idx, info_string, 0, false )
        reaper.GetSetAutomationItemInfo(env, ai_destino_idx, info_string, ai_origem_value, true )
    end
end
