...
function is.OpenaAllSelectedMediaAsSequencer(proj, is_item, version)
    local seqs = {}
    local func = (is_item and DL.enum.SelectedMediaItem(proj)) or DL.enum.SelectedTracks2(proj, false)
    for seq in func do 
        local st = is.OpenSequencer(proj, seq, is_item, version)
        if st then
            seqs[#seqs+1] = st
        end
    end
...
function is.ReopenSequencers(proj, sequencers, version)
    for k, sequencer in DL.t.ipairs_reverse(sequencers) do
        ...
        sequencers[k] = is.OpenSequencer(proj, sequencer.seq, sequencer.is_item, version)
...
function is.OpenSequencer(proj, seq, is_item, version)
    ...
    if not st then
        ...
        if is_item then
            ...
            if s_guid ~= '' then
                print("Can't make an item generated by Item Sampler into a sequencer item!")
                return false
            end
        end
        -- Make it 
        st = is.MakeSequencer(proj, seq, is_item, version)
    end

    UpdateVersion = is.UpdateSequencerVersion(st)
    return st
end

function is.MakeSequencer(proj, seq, is_item, version)
    local st = {
        groups = {
...
function is.UpdateSequencerVersion(st)
    if not st.version then -- before 1.8.0
        local new_group = BlankGroup:Create('whatever')
        for gi, group in ipairs(st.groups) do -- add stretch configs
            group.Settings.stretch = table_copy(new_group.Settings.stretch)
        end
        st.version = '1.8.0'
    end
   return st 
