...
function AddToGroup(group)
    local selected_items = CreateSelectedItemsTable()
    for k, item in ipairs(selected_items) do
        for take in enumTakes(item) do
            group[#group+1] = {take = take, chance = 1}
        end
    end    
end
...
function CreateProjectConfigTable(project)
    local t = {
        ...
        oldpos = reaper.GetPlayPositionEx( project ), 
        oldtime = reaper.time_precise(),
        oldisplay = reaper.GetPlayStateEx(project)&1 == 1,
